...
1. Переменные (Variables).
	a. Имя, которому присваивается значение или имя, связанное с данными, которые можно использовать в программе.
	b. Автоматическая типизация.
x = 10 # автоматически определил тип как int
name = "Alice" # автоматически определил тип как str

2. Типы данных (Data Types):
	a. Неизменяемые (immutable). Значение изменить нельзя — можно только создать новое.
	...
	b. Изменяемые типы данных (mutable). Можно менять содержимое без создания нового объекта.
		| Тип                                           | Eng        | Описание                 | Пример                                                                                                                                                                 | Подсказка                                                                               |
		|:----------------------------------------------|:-----------|:-------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------|
...
3. Операторы (Operators).
	a. Арифметические (Arithmetic). Позволяют выполнять математические вычисления.
	...
	b. Сравнения (Comparison). Сравнивают два значения, возвращают True или False.
		* == - равно (Equal to)
	...
	c. Логические (Logical). Работают с логическими значениями True и False.
		* and - истина, если оба выражения истинны (И)
		* or - истина, если хотя бы одно истинно (ИЛИ)
		* not - инвертирует логическое значение (НЕ)
	d. Присваивания (Assignment). Присваивают значение переменной.
	...
	e. Побитовые (Bitwise). Выполняют операции над битами целых чисел.
	...
	f. Принадлежности (Membership). Проверяют, принадлежит ли элемент коллекции.
		* in	- проверяет, содержится ли элемент (In)
		* not in - проверяет, не содержится ли элемент (Not in)
	g. Идентичности (Identity). Сравнивают, являются ли два объекта одним и тем же в памяти.
		* is	- Истина, если оба объекта — один и тот же объект (Is)
		* is not - Истина, если объекты — разные (Is not)

4. Управляющие конструкции.
	a. Условные конструкции (Conditional Statements). if, elif, else. Позволяют выполнять код в зависимости от условий.
if x > 0:
    print("Positive")
elif x == 0:
    print("Zero")
...
5. Функции (Functions).
	a. Определение функции (Function Definition)
def greet(name):
    return f"Hello, {name}!"
	b. Аргументы функций (Function Arguments).
	...
	c. Лямбда-функция (Lambda Function). Анонимная однострочная функция.
add = lambda x, y: x + y
	d. Встроенные функции (Built-in Functions).
...
6. Коллекции и методы (Collections & Their Methods). Коллекции — это структуры данных, которые могут хранить несколько значений.
	a. Список (List). Упорядоченная изменяемая коллекция (можно менять, добавлять, удалять элементы). Допускает дубликаты, может содержать любые типы.
	...
	b. Словарь (Dictionary). Структура "ключ: значение". Порядок сохраняется с Python 3.7. Ключи должны быть уникальными и неизменяемыми (например, строки, числа, кортежи).
	...
	c. Множество (Set). Неупорядоченная коллекция уникальных элементов. Автоматически удаляет повторы.
	...
	d. Строка (String). Неизменяемая последовательность символов → методы возвращают новую строку.
...
7. Классы и ООП (Classes and OOP). ОПП (OOP — Object-Oriented Programming) — это способ организации кода, где данные и функции объединяются в объекты.
	a. Класс (Class). Шаблон (чертёж), по которому создаются объекты.
		* Определяет:
			* какие данные объект будет хранить
			* какие действия он сможет выполнять
class Person:
    pass
	b. Объект (Object / Instance). Экземпляр (конкретный представитель) класса. Создаётся из класса.
p = Person()   # объект класса
	c. Конструктор (Constructor / Initializer). специальный метод, который выполняется при создании объекта. В Python это init.
class Person:
    def __init__(self, name):
        self.name = name
	d. Атрибут (Attribute / Property / Field). Переменные, которые принадлежат объекту или классу.
		* self.name — атрибут объекта
		* ClassName.value — атрибут класса
self.age = 20
	e. Метод (Method). Функция внутри класса, которая принадлежит объекту.
def say_hello(self):
    print("Hello")
	f. Наследование (Inheritance). Возможность создать новый класс на основе другого, позволяет переиспользовать код.
class Student(Person):
    pass
	g. Инкапсуляция (Encapsulation). Cокрытие внутренних данных/методов от прямого доступа.
	...
	h. Полиморфизм (Polymorphism). Возможность использовать один и тот же метод в разных классах, но с разным поведением.
class Cat:
    def sound(self):
        print("Meow")
...
for animal in [Cat(), Dog()]:
    animal.sound()
	i. Магические методы (Magic methods / Dunder methods). Cпециальные методы с имя, позволяют изменять стандартное поведение объектов
		* init — конструктор
		* str — текстовое представление
		* repr — "официальное" представление
		* len — поведение функции len()
		* getitem — доступ по индексу
		* add — поведение оператора +
def __str__(self):
    return f"Person: {self.name}"
	j. Атрибуты класса (Class Attributes). Переменные, общие для всех объектов класса.
class Person:
    species = "Human"   # атрибут класса
	k. Атрибуты объекта (Instance Attributes). Переменные, уникальные для каждого объекта.
self.name = "Alice"
	l. Статический метод (Static Method). Метод, который не использует ни объект (self), ни класс (cls) - просто функция внутри класса.
@staticmethod
def add(a, b):
    return a + b
	m. Метод класса (Class Method). Метод, который работает с классом, а не с объектом, принимает cls.
@classmethod
def create_default(cls):
    return cls("NoName")
	n. Абстрактный класс (Abstract Class). Класс, от которого нельзя создать объект, только унаследоваться; используется как шаблон.
...
s = Square()
print(s.area())  # 25
	o. Интерфейс (Interface). Набор методов, которые класс должен реализовать (без их реализации). В Python используется абстракция через abc.
# То же, что выше: абстрактный метод = обязательный к реализации
	p. Композиция (Composition). Класс содержит объект другого класса.
class Car:
    def __init__(self):
        self.engine = Engine()
	q. Утиная типизация (Duck Typing). Если что‑то выглядит как утка и крякает — значит, это утка". Главное — поведение, а не тип.
class Duck:
    def quack(self):
        print("Quack!")
...
make_quack(Person())   # I'm pretending to be a duck
															 ...
															 │                                        │
												   ┌───────▼────────┐        ┌───────▼────────┐
												   │        Instance Attr	         │        │        Class Attribute        │
												   └────────────────┘        └─────────────────┘
											
													Object uses:
													* Methods (instance, static, class)
													* Magic Methods (e.g. str, add)
													* Encapsulation (private/protected/public)
													* Composition (has other objects inside)
													* Inheritance (inherits from another class)
													Polymorphism applies when:
													* Multiple classes implement same method name with different behavior
...
8. Модули и пакеты (Modules & Packages). 
	...
	* Package — это папка с модулями, которая имеет файл init.py внутри. Пакет — это способ организовать модули в структуру, как "папки с кодом".
	* init.py — что это? Этот файл делает папку пакетом. Может быть пустым, но иногда туда пишут код, который запускается при импорте пакета.
	* Как создать модуль:
	...
	* Стандартные модули (stdlib).
	...
	a. Импорт модулей (Importing Modules)
import math
from os import path
	...
	b. Установка пакетов (Installing Packages). Если модуль не входит в стандартную библиотеку, его нужно установить через pip. Если ты пишешь свою библиотеку и хочешь установить её через pip, нужна структура с setup.py.
pip install numpy
	c. Создание собственного модуля (Creating Your Own Module).
...
9. Исключения (Exceptions). Это ошибка, которая возникает во время выполнения программы. Чтобы программа не "падала", Python позволяет ловить и обрабатывать такие ошибки.
	a. Try/Except. Основной механизм обработки ошибок. except (несколько исключений)
try:
...
try:
    ...
    print("Ошибка!")
	b. Raise. Используется, когда нужно остановить программу или сообщить об ошибке.
age = -5
if age < 0:
    raise ValueError("Возраст не может быть отрицательным")
	c. Else. Выполняется, если ошибок не было
try:
    x = 10 / 2
	...
	d. Finally. Выполняется ВСЕГДА. Чаще всего используется для освобождения ресурсов.
try:
    file = open("data.txt")
    data = file.read()
    ...
    file.close()
	e. Создание собственных исключений. Можно создать свой класс ошибки.
...
file = open("имя_файла", "режим")
	...
	b. Чтение файла. Всегда закрывай файл f.close() — или лучше используй with.
f = open("file.txt", "r")
...
f.close()                 # Закрывает файл
	c. Запись в файл. Если файла не существует — он создаётся. Если существует — будет перезаписан.
f = open("file.txt", "w")
f.write("Hello, world!")
f.close()
	d. Добавление. Новый текст добавляется в конец, не удаляя предыдущие данные.
f = open("file.txt", "a")
f.write("\nNew line")
f.close()
	e. Контекстный менеджер with. Это лучший способ работы с файлами. Файл автоматически закрывается, даже если произошла ошибка.
with open("file.txt", "r") as f:
    text = f.read()
    print(text)

11. Итераторы и генераторы (Iterators & Generators).
	a. Итератор (Iterator). Это объект, по которому можно пройтись по одному элементу (в любых коллекциях: списки, строки, словари, множества) за раз (например, в for-цикле, он вызывается атоматически). Имеет методы iter() и next() Возвращает следующий элемент при каждом вызове next(). Если элементов больше нет — next() вызовет StopIteration.
numbers = [1, 2, 3]
...
hasattr(obj, "__next__")   # объект — итератор
	b. Генераторы (Generators). Это особый тип итератора, который создаётся функцией с ключевым словом yield. Он возвращает значения по одному, не храня их все в памяти. Работает с большими данными (например, миллионы строк), экономит память, позволяет делать ленивые вычисления (lazy evaluation).
def count_up_to(n):
    i = 1
    while i <= n:
        yield i
...
15. Дополнительные (продвинутые) темы.
	a. Метаклассы (Metaclasses). Управляют созданием классов (родвинутый уровень ООП).
class Meta(type):
    def __new__(cls, name, bases, dct):
        return super().__new__(cls, name, bases, dct)
	b. Дескрипторы (Descriptors). Механизм управления доступом к атрибутам объекта.
class Descriptor:
    def __get__(self, obj, objtype): ...
	c. Типизация и аннотации (Typing / Annotations). Повышают читаемость и помогают IDE.
def add(x: int, y: int) -> int:
    return x + y
	d. Быстрая генерация классов данных (Dataclasses).
...
match value:
    ...
    case _:
        print("Default")
	f. Корутины и событийные циклы (Coroutines and Event Loops). Глубже, чем просто async/await. Взаимодействие с asyncio, aiohttp, uvicorn и пр.
	g. Packaging / Environment Tools. Не про сам язык, но важно:
		* venv — виртуальные окружения
		* pip, pyproject.toml, setuptools, poetry — управление зависимостями
		* pytest — тестирование
	h. Многозадачность на уровне процессов и потоков (Concurrency & Multiprocessing).
from threading import Thread
from multiprocessing import Process

16. Дополнительные (экспертные) темы.
	a. Архитектура фреймворков (Framework Architecture)
		* Назначение – понимание, как устроены Django, FastAPI, Flask и как создать нечто подобное.
			* Что используется:
				...
				* Динамический импорт → import(), importlib
...
def hello():
    return "Hello world"
	b. Внедрение зависимостей (Dependency Injection).
...
def resolve(func):
	...
	c. Инспекция и рефлексия (Introspection & Reflection)
...
print(inspect.signature(func))  # (x, y)
	d. Генерация кода во время выполнения (Code Generation & Eval).
...
x = 10
print(eval(code))  # 11
	e. Байткод и дизассемблирование (Bytecode & Disassembly).
...
dis.dis(add)
	f. Профилирование и оптимизация (Performance Profiling & Optimization).
		* Инструменты:
	...
	g. Модули на C (Writing C Extensions).
...
static PyObject* say_hello(PyObject* self, PyObject* args) {
	...
	h. Внутренности асинхронности (Async Internals).
		* Что изучать:
			* asyncio event loop
			* Futures и Tasks
	...
	i. Память и сборка мусора (Memory Model & GC).
		* Что важно:
		...
		* sys.getrefcount()
	j. GIL и многопоточность (GIL & Multithreading).
...
sys.meta_path.insert(0, MyFinder())
	b. Собственный байткод-интерпретатор.
		* Что это:
	...
	c. Изменение синтаксического дерева (Abstract Syntax Tree (AST) Manipulation).
		* Что это:
...
tree = ast.parse("x = 1 + 2")
	d. Writing Your Own Language on Top of Python.
		* Что это:
	...
	e. Alternative Python Implementations.
		* Что это:
	...
	f. Building Your Own Python Interpreter.
		* Что это:
	...
	g. Working on the Python Language Itself (CPython Core Dev).
		* Что включает:
	...
	h. Meta-circular Python (Python, написанный на Python).
		* Что это:
	...
	i. PEP Process & Language Governance.
		* Что это:
	...
	j. Python VM Hacking (Изменение CPython в C). Это уже почти уровень разработки нового языка программирования.
		* Что это:
